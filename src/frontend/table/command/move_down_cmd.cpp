...
public:
    Cmd(TableView *table, const TableCmd::Range &range, QVariant &&old, QVariant &&cur, const int insertCount)
        : QUndoCommand("MoveDown"), m_table(table), m_range(range), m_old(std::move(old)),
    ...
    void undo() override
    {
        ...
        if (m_insertCount > 0) {
            model->removeRows(model->rowCount({}) - m_insertCount, m_insertCount, {});
        }
    ...
    void redo() override
    {
        ...
        const int rCount = m_range.bottom - m_range.top + 1;
        if (m_insertCount > 0) {
            model->insertRows(model->rowCount({}), m_insertCount, {});
        }
        for (int c{0}; c < cCount; ++c) {
            const auto &d = data.at(c);
            for (int r{0}; r < rCount; ++r) {
...
MoveDownCmd::MoveDownCmd(TableView *table) : TableCmd(table)
{
}

void MoveDownCmd::cmd(QObject *contextObject, const QItemSelection &selectionItem)
{
    ...
    // 获取数据，并插入数据
    int maxRow = lastRowIndex - range.top + 1;
    ...
    for (qsizetype i{0}; i < sortSelection.size(); ++i) {
        ...
        for (int col{curRange.left()}; col <= curRange.right(); ++col) {
            ...
            curData[j].insert(curRange.top() - range.top, curRange.height(), {});
            maxRow = qMax(maxRow, static_cast<int>(curData[j].size()));
        }
    }
    // 计算是否需要插入行
    const int newTableRowCount = maxRow + range.top;
    const int insertCount = newTableRowCount > model->rowCount() ? newTableRowCount - model->rowCount() : 0;
    // 更新底部索引，并对齐数据
