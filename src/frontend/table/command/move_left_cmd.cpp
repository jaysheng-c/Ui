...
public:
    Cmd(TableView *table, const TableCmd::Range &range, QVariant &&old, QVariant &&cur)
        : QUndoCommand("MoveLeft"), m_table(table), m_range(range), m_old(std::move(old)),
        m_cur(std::move(cur))
    {
    }
...
MoveLeftCmd::MoveLeftCmd(TableView *table) : TableCmd(table)
{
}

void MoveLeftCmd::cmd(QObject *contextObject, const QItemSelection &selectionItem)
{
    ...
    // 获取数据，并删除数据
    const int maxColumn = lastColIndex - range.left + 1;
    ...
    for (qsizetype i{0}; i < sortSelection.size(); ++i) {
        ...
        for (int row{curRange.top()}; row <= curRange.bottom(); ++row) {
            ...
            curData[j].remove(curRange.left() - range.left, curRange.width());
        }
    }
    // 更新右侧索引，并对齐数据
